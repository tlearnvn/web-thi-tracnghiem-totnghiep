...
final class Icons
{
    ...
    public static function svg(string $name, string $class = '', int $size = 0): string
    {
        $inner = self::MAP[$name] ?? self::MAP['circle'];
        $attr = $size > 0 ? ' width="' . $size . '" height="' . $size . '"' : '';
        return '<svg class="ic' . ($class !== '' ? ' ' . $class : '') . '"' . $attr . ' viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' . $inner . '</svg>';
    }
}
